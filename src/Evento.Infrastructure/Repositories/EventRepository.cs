...
namespace Evento.Infrastructure.Repositories
{
    public class EventRepository : IEventRepository
    {
        ...
        public async Task<Event> GetAsync(string name)
            => await Task.FromResult(_events.SingleOrDefault(x=>x.Name.ToLowerInvariant().Equals(name.ToLowerInvariant())));

        public async Task<IEnumerable<Event>> BroseAsync(string name = "")
        {
